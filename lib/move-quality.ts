...
/** Conservative material-offer test, not Chess.com's unpublished classifier.
 * Require a legal capture of the offered piece and a material deficit even
 * after the strongest immediate recapture. Pins and illegal king captures
 * cannot establish a sacrifice. Pawn offers and indirect offers are excluded.
 */
function isSacrificialMove(fenBefore: string, moveUci: string | null): boolean {
  ...
  try {
    ...
    const offered = game.move(parsed);
    if (offered.piece === "p" || offered.piece === "k" || game.isGameOver()) return false;
    const invested = PIECE_VALUES[offered.piece] - (offered.captured ? PIECE_VALUES[offered.captured] : 0);
    if (invested < 2) return false;
    return game.moves({ verbose: true }).filter(move => move.to === offered.to && move.captured).some(capture => {
      const accepted = new Chess(game.fen());
      accepted.move(capture);
      const refund = Math.max(0, ...accepted.moves({ verbose: true }).filter(move => move.to === offered.to && move.captured).map(move => PIECE_VALUES[move.captured!]));
      return invested - refund >= 2;
    });
  } catch { return false; }
}
...
export function isBrilliantMove(args: {
  ...
  if (![cpLoss, evalBeforeMover, evalAfterMover].every(Number.isFinite) || cpLoss < 0) return false;
  // Local, conservative thresholds; Chess.com does not publish equivalent cp cutoffs.
  if (cpLoss > 10 || (!isBestMove && cpLoss > 5)) return false;
  if (evalAfterMover < -50) return false;
  // Compare another real candidate when available, rather than rejecting the
  // only winning sacrifice just because the best root evaluation is high.
  if ((args.alternativeEvalMover ?? evalBeforeMover) >= 300) return false;
  return isBrilliantCandidate(fenBefore, moveUci, engineDepth);
}
...
export function buildMoveQualityCommentary(args: {
  ...
  switch (args.classification) {
    case "brilliant":
      return "A sound piece sacrifice that stays close to the engine's best move.";
    case "best":
