...
print(f'Mix peak: {20*math.log10(peak*gain):.2f} dBFS; duration 24s')
subprocess.run([str(ffmpeg),'-y','-v','error','-i','chaos-output/chaos-chess-anarchy-vertical.mp4','-i',str(root/'narrated-mix.wav'),'-map','0:v:0','-map','1:a:0','-c:v','copy','-c:a','aac','-b:a','192k','-movflags','+faststart','-t','24','chaos-output/chaos-chess-anarchy-leda.mp4'],check=True)
print('Finished narrated MP4; original silent-caption version preserved.')
