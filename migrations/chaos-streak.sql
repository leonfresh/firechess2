-- Chaos Chess daily streaks. Builds on migrations/chaos-gold.sql (apply that first). This file
-- redefines archive_chaos_match() with the gold body unchanged plus a streak bonus.
--   node scripts/chaos-career-migrate.mjs migrations/chaos-streak.sql
--
-- Rules (mirrored for display in lib/chaos-streak.ts, keep the two in step)
--   A streak day is an Australia/Sydney day with at least one 'match' gold row, so only registered
--   identities count and the 8-paying-games-per-pair cap still applies. The first paying game of
--   each day pays a 'streak' bonus of 10 gold per consecutive day beyond the first, capped at 50:
--   day 2 = +10, day 3 = +20 ... day 6 onward = +50. Missing a Sydney day resets the run to 1.
--   Mastery counts only 'match' rows, so the bonus never inflates collection frames.

-- True when the player already earned match gold on the given Sydney calendar day.
CREATE OR REPLACE FUNCTION chaos_paid_on_day(pid text, d date) RETURNS boolean LANGUAGE sql STABLE AS $$
 SELECT EXISTS(SELECT 1 FROM chaos_gold_ledger WHERE player_id = pid AND reason = 'match'
  AND created_at >= (d::timestamp AT TIME ZONE 'Australia/Sydney')
  AND created_at < ((d + 1)::timestamp AT TIME ZONE 'Australia/Sydney'))
$$;

-- Consecutive Sydney days, ending today, with match gold. Bounded so a long run stays cheap.
CREATE OR REPLACE FUNCTION chaos_streak_length(pid text) RETURNS integer LANGUAGE plpgsql STABLE AS $$
DECLARE d date := (now() AT TIME ZONE 'Australia/Sydney')::date; n integer := 0;
BEGIN
 WHILE n < 366 AND chaos_paid_on_day(pid, d) LOOP
  n := n + 1; d := d - 1;
 END LOOP;
 RETURN n;
END $$;
