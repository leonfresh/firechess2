...
const gameRe = /id:\s*"([\w-]+)"[\s\S]*?fen:\s*"([^"]+)"[\s\S]*?moves:\s*"([^"]+)"/g;
let m;
...
for (const { id, fen, moves } of entries) {
  ...
  const mv = moves.split(',').map(x => x.trim()).filter(Boolean);
  let failIdx = -1;
  const valid = [];
  for (let j = 0; j < mv.length; j++) {
    try {
      const r = chess.move(mv[j]);
      if (!r) { failIdx = j; break; }
      valid.push(mv[j]);
    } catch {
      failIdx = j;
      break;
  ...
  if (failIdx === -1) {
    ...
    console.log(`✗ ${id} — fails at [${failIdx}]/${mv.length}: "${mv[failIdx]}"`);
