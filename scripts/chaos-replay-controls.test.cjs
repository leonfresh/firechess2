...
test('finished replay can play again, rewind, and play a second time',()=>{
 ...
 vm.runInNewContext(ts.transpile(fs.readFileSync('components/chaos-watch.tsx','utf8'),{module:ts.ModuleKind.CommonJS,jsx:ts.JsxEmit.ReactJSX,target:ts.ScriptTarget.ES2022,esModuleInterop:true}),{module,exports:module.exports,require:n=>n==='react'?react:n==='react/jsx-runtime'?require(n):n==='chess.js'?require(n):n.includes('chaos-watch')?{describeWatchFrame:f=>f.label,describeWatchPower:()=>null,expandVisual:s=>({...s,playerModifiers:[],aiModifiers:[]})}:n.includes('chaos-pieces')?{buildChaosCustomPieces:()=>({})}:n.includes('chaos-anomalies')?{getAnomalyById:()=>undefined}:{},Date,URL});
