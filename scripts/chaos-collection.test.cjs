...
function load(){
 let cursor=0,lastDeps;const values=[];
 const react={...require('react'),useId:()=>"collection-test",useEffect(fn,deps){if(JSON.stringify(deps)!==lastDeps){lastDeps=JSON.stringify(deps);try{fn()}catch{}}},useRef(v){return{current:v}},useMemo(fn){return fn()},useState(v){const i=cursor++;if(!(i in values))values[i]=v;return[values[i],n=>values[i]=typeof n==='function'?n(values[i]):n]}};
 const module={exports:{}};
 const shims={
  '@/lib/chaos-chess':{ACTIVE_MODIFIERS:MODS},
 ...
 vm.runInNewContext(ts.transpile(fs.readFileSync('discord-activity/app/collection.tsx','utf8'),{module:ts.ModuleKind.CommonJS,jsx:ts.JsxEmit.ReactJSX,target:ts.ScriptTarget.ES2022,esModuleInterop:true}),
  {module,exports:module.exports,require:name=>shims[name]??(name==='react'?react:name==='react/jsx-runtime'?require(name):{}),Date,URL,Math,console,setTimeout,clearTimeout,Set,Promise,Event:class {},window:{dispatchEvent(){}},
   fetch:async (url,init)=>String(url).includes('/api/chaos/shop')
    ...
    :{ok:true,json:async()=>String(url).includes('collection')?{unlockedIds:['alpha'],total:MODS.length,gold:590,goldWeek:55,shop:SHOP}:{profile:{games:2}}}});
...
test('every tab is reachable and the card is a button, not a link',()=>{
 ...
 assert.deepEqual(tabs,['Powers (3)','Pieces (11)','Anomalies (1)']);
 assert.ok(nodes.some(n=>n.props?.href==='/shop'),'shop opens its dedicated page');
...
test('the armoury shows the gold balance the player has earned',async()=>{
 ...
 assert.match(pill.props['aria-label'],/^590 gold, 55 earned this week$/);
...
test('the shop tab prices locked cards and buying one adds it to the kit',async()=>{
 const {exports,reset}=load();
 exports.ActivityCollection({page:true,shopEntry:true});
 await new Promise(r=>setTimeout(r,0));
 reset();
 let tree=walk(exports.ActivityCollection({page:true,shopEntry:true}));
 assert.ok(!tree.some(n=>n.type==='dialog'),'the shop is a page');
 const body=texts(tree);
 assert.ok(tree.some(n=>n.type==='b' && n.props.children?.includes(400)),'a locked card shows its server price');
 assert.ok(body.includes('Does beta things.'),'and what the card does');
 const buy=tree.find(n=>n.props?.className==='shop-buy');
 assert.ok(buy,'a locked card offers a buy button');
 buy.props.onClick();
 reset();
 tree=walk(exports.ActivityCollection({page:true,shopEntry:true}));
 assert.ok(texts(tree).includes("Confirm unlock"));
 await tree.find(n=>n.props?.children==="Confirm unlock").props.onClick();
 reset();
 tree=walk(exports.ActivityCollection({page:true,shopEntry:true}));
 assert.ok(texts(tree).includes('✓ Ready for your drafts'),'after buying, the card is owned');
