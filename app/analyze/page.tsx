            ...
            const fl = probe
              ...
              .filter(
                (m) =>
                  m.piece === fpt &&
                  m.to === fp.target &&
                  (!fp.promotion || m.promotion === fp.promotion),
              );
            if (fl.length >= 1) {
              try {
                probe.move(fl[0].san);
  ...
  try {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      try {
        ...
        if (!result) {
          return {
            error: `Illegal move "${token}" at move ${fullMove}${color === "w" ? "" : "..."} — the position does not allow this move.`,
          };
        }
        ...
        try {
          const resolved = resolveAmbiguousMove(
            chess,
            token,
            tokens.slice(i + 1),
          );
        ...
        return {
          error: `Invalid move "${token}" at move ${fullMove}${color === "w" ? "" : "..."} — the PGN may be corrupted or use a non-standard format.`,
        };
      }
    }
    ...
    return {
      error: "Could not parse PGN. Please check the format and try again.",
    };
  }
}
...
export default function AnalyzePage() {
  ...
  useEffect(() => {
    try {
      ...
      const shouldAutostart =
        sessionStorage.getItem("firechess-analyze-autostart") === "workbench";
      if (stored) {
        setPgnText(stored);
      ...
      if (shouldAutostart) {
        ...
        sessionStorage.removeItem("firechess-analyze-autostart");
      }
    } catch {
  ...
  const isSequentialNav =
    Math.abs(selectedMoveIdx - prevMoveIdxRef.current) <= 1;
  useEffect(() => {
    prevMoveIdxRef.current = selectedMoveIdx;
  }, [selectedMoveIdx]);
  ...
  const currentFen = useMemo(() => {
    if (selectedMoveIdx < 0) {
      ...
      return analyzedMoves.length > 0
        ? analyzedMoves[0].fenBefore
        : "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1";
    }
    return (
      analyzedMoves[selectedMoveIdx]?.fenAfter ??
      "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"
    );
  ...
  const filteredLibraryGames = useMemo(() => {
    ...
    if (libraryCategory !== "all")
      games = games.filter((g) => g.category === libraryCategory);
    if (libraryTagFilter !== "all")
      games = games.filter((g) => g.tags.includes(libraryTagFilter as any));
    return games;
  ...
  const summary = useMemo(() => {
    ...
    const accuracy = (acpl: number) =>
      Math.min(99.5, Math.max(25, 100 * Math.exp(-acpl / 120)));

    const count = (moves: AnalyzedMove[], cls: MoveClassification) =>
  ...
  const analyzeGame = useCallback(async () => {
    ...
    const startEval = await stockfishPool.evaluateFen(
    ...
    for (let i = 0; i < parsed.moves.length; i++) {
      ...
      const bestResult = await stockfishPool.evaluateFen(
        move.fenBefore,
        engineDepth,
      );
      ...
      const evalAfterWhite = evalResult
        ? move.color === "w"
          ? -evalResult.cp
          : evalResult.cp
        : prevEvalWhite;
      ...
      const isBestMove =
        bestResult?.bestMove === move.uci ||
        (bestResult?.bestMove &&
          move.uci.startsWith(bestResult.bestMove.slice(0, 4)) &&
          cpLoss <= 5);
  ...
  useEffect(() => {
    ...
    void analyzeGame();
  }, [pendingAutostart, pgnText, analyzing, analyzedMoves.length, analyzeGame]);
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowLeft") {
        ...
        e.preventDefault();
        setSelectedMoveIdx((prev) =>
          Math.min(analyzedMoves.length - 1, prev + 1),
        );
      } else if (e.key === "Home") {
        e.preventDefault();
        setSelectedMoveIdx(-1);
  ...
  const openExplain = useCallback(
    (move: AnalyzedMove, tab: "played" | "best") => {
      ...
      const playedTitle = (() => {
        if (move.classification === "blunder")
          return `Why ${move.san} is a blunder`;
        if (move.classification === "mistake")
          return `Why ${move.san} is a mistake`;
        if (move.classification === "inaccuracy")
          return `Why ${move.san} is inaccurate`;
        if (move.classification === "brilliant")
          return `Why ${move.san} is brilliant`;
        if (move.classification === "best")
          return `Why ${move.san} is the best move`;
        if (move.classification === "good")
          return `Why ${move.san} is a good move`;
    ...
    return (
      <svg
        ...
        onClick={(e) => {
          ...
          setSelectedMoveIdx(
            Math.max(-1, Math.min(analyzedMoves.length - 1, idx)),
          );
        }}
      >
        {/* Background */}
          ...
          points={`0,${h / 2} ${points
            .map((p, i) => {
              const clamped = Math.max(
                -maxCp,
                Math.min(maxCp, evalGraph[i].cp),
              );
              const y = h / 2 - (clamped / maxCp) * (h / 2);
              return y < h / 2
                ? p
                : `${(i / (evalGraph.length - 1)) * w},${h / 2}`;
            })
            .join(" ")} ${w},${h / 2}`}
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-slate-400">
            Paste a PGN to get move-by-move analysis with Stockfish — entirely
            in your browser
          </p>
        </div>
          ...
          <div className="mx-auto max-w-2xl space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              >
                ...
                <p className="text-sm font-bold text-blue-400 group-hover:text-blue-300">
                  Load from Lichess / Chess.com
                </p>
                <p className="mt-1 text-[11px] text-slate-500">
                  Import your recent games
                </p>
              </button>
              ...
              >
                <span className="mb-2 flex justify-center text-2xl">📚</span>
                <p className="text-sm font-bold text-amber-400 group-hover:text-amber-300">
                  Browse Game Library
                </p>
                <p className="mt-1 text-[11px] text-slate-500">
                  {SAMPLE_GAMES.length} famous games across{" "}
                  {GAME_CATEGORIES.length} collections
                </p>
              </button>
            </div>
          </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {summary.white.brilliant > 0 && (
                      <span className="rounded-full bg-cyan-500/15 px-2 py-0.5 text-[11px] font-bold text-cyan-400">
                        !! {summary.white.brilliant}
                      </span>
                    )}
                    {summary.white.best > 0 && (
                    ...
                    {summary.white.inaccuracy > 0 && (
                      <span className="rounded-full bg-amber-500/15 px-2 py-0.5 text-[11px] font-bold text-amber-400">
                        ?! {summary.white.inaccuracy}
                      </span>
                    )}
                    {summary.white.mistake > 0 && (
                      <span className="rounded-full bg-orange-500/15 px-2 py-0.5 text-[11px] font-bold text-orange-400">
                        ? {summary.white.mistake}
                      </span>
                    )}
                    {summary.white.blunder > 0 && (
                ...
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {summary.black.brilliant > 0 && (
                      <span className="rounded-full bg-cyan-500/15 px-2 py-0.5 text-[11px] font-bold text-cyan-400">
                        !! {summary.black.brilliant}
                      </span>
                    )}
                    ...
                    {summary.black.good > 0 && (
                      <span className="rounded-full bg-emerald-500/10 px-2 py-0.5 text-[11px] font-medium text-emerald-400/70">
                        Good {summary.black.good}
                      </span>
                    )}
                    {summary.black.inaccuracy > 0 && (
                      <span className="rounded-full bg-amber-500/15 px-2 py-0.5 text-[11px] font-bold text-amber-400">
                        ?! {summary.black.inaccuracy}
                      </span>
                    )}
                    {summary.black.mistake > 0 && (
                      <span className="rounded-full bg-orange-500/15 px-2 py-0.5 text-[11px] font-bold text-orange-400">
                        ? {summary.black.mistake}
                      </span>
                    )}
                    {summary.black.blunder > 0 && (
                      <span className="rounded-full bg-red-500/15 px-2 py-0.5 text-[11px] font-bold text-red-400">
            ...
            <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_280px]">
              ...
              <div className="flex flex-col items-center gap-3">
                <div ref={boardRef} className="w-full max-w-[640px]">
                  <div className="flex items-start gap-2 sm:gap-3">
                    ...
                    <div className="overflow-hidden rounded-xl">
                      <Chessboard
                        key={
                          isSequentialNav ? "seq" : `jump-${selectedMoveIdx}`
                        }
                        id="analyze-board"
                        position={currentFen}
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <button
                    onClick={() =>
                      setSelectedMoveIdx((p) => Math.max(-1, p - 1))
                    }
                    disabled={selectedMoveIdx <= -1}
                    className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/[0.06] text-slate-400 transition-colors hover:bg-white/[0.12] hover:text-white disabled:opacity-30 disabled:cursor-not-allowed"
                    title="Previous move (←)"
                  ...
                  <button
                    onClick={() =>
                      setSelectedMoveIdx((p) =>
                        Math.min(analyzedMoves.length - 1, p + 1),
                      )
                    }
                    disabled={selectedMoveIdx >= analyzedMoves.length - 1}
                    className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/[0.06] text-slate-400 transition-colors hover:bg-white/[0.12] hover:text-white disabled:opacity-30 disabled:cursor-not-allowed"
                    title="Next move (→)"
                  >
                    <svg
                  ...
                  <button
                    onClick={() => setSelectedMoveIdx(analyzedMoves.length - 1)}
                    disabled={selectedMoveIdx >= analyzedMoves.length - 1}
                    className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/[0.06] text-slate-400 transition-colors hover:bg-white/[0.12] hover:text-white disabled:opacity-30 disabled:cursor-not-allowed"
                    title="Last move (End)"
                  >
              ...
              {/* Move list column */}
              <div
                className="rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden flex flex-col w-full lg:w-auto"
                style={{ maxHeight: boardSize + 120 }}
              >
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      while (i < analyzedMoves.length) {
                        const whiteMove =
                          analyzedMoves[i]?.color === "w"
                            ? analyzedMoves[i]
                            : null;
                        const blackMove = whiteMove
                          ? analyzedMoves[i + 1]?.color === "b"
                            ? analyzedMoves[i + 1]
                            : null
                          : analyzedMoves[i]?.color === "b"
                            ? analyzedMoves[i]
                            : null;
                        const moveNum =
                          whiteMove?.moveNumber ?? blackMove?.moveNumber ?? 0;
                      ...
                      .map((m) => (
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            <p
                              className={`text-sm font-bold ${CLASSIFICATION_COLORS[m.classification]}`}
                            >
                              {m.moveNumber}
                              {m.color === "w" ? "." : "..."}
                              {m.san}
                              <span className="ml-1.5 text-xs text-slate-500 font-normal capitalize">
                                {m.classification}
                              </span>
                            </p>
                            <p className="text-[11px] text-slate-500">
                              {formatEval(m.evalBefore)} →{" "}
            ...
            <div className="space-y-4">
              <div className="flex gap-2">
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Enter") fetchRecentGames();
                  }}
                  placeholder={
                    loadSource === "lichess"
                      ? "Lichess username"
                ...
                <div className="space-y-1.5 max-h-80 overflow-y-auto rounded-xl border border-white/[0.06] bg-black/30 p-2">
                  {recentGames.map((game, idx) => (
                    ...
                    >
                      ...
                      <span
                        className={`shrink-0 rounded-full border px-2 py-0.5 text-[10px] font-bold ${game.result === "1-0" ? "border-white/10 text-white" : game.result === "0-1" ? "border-slate-600 text-slate-400" : "border-slate-700 text-slate-500"}`}
                      >
                        {game.result}
                      </span>
                    </button>
              ...
              {recentGames.length === 0 && !loadingGames && !loadError && (
                <p className="text-center text-xs text-slate-600 py-4">
                  Enter your username and hit Load to see your last 10 games
                </p>
              )}
            </div>
          </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-5 custom-scrollbar">
                ...
                <div className="sm:hidden mb-4">
                  <select
                    ...
                    onChange={(e) =>
                      setLibraryCategory(e.target.value as GameCategory | "all")
                    }
                    className="w-full rounded-xl border border-white/[0.08] bg-black/40 px-3 py-2 text-sm text-slate-200 outline-none"
                  >
                    <option value="all">
                      📚 All Games ({SAMPLE_GAMES.length})
                    </option>
                    {GAME_CATEGORIES.map((cat) => (
                      <option key={cat.key} value={cat.key}>
                        {cat.icon} {cat.label} (
                        {
                          SAMPLE_GAMES.filter((g) => g.category === cat.key)
                            .length
                        }
                        )
                      </option>
                ...
                {libraryCategory !== "all" && (
                  <div className="mb-4 rounded-xl border border-white/[0.06] bg-white/[0.02] px-4 py-3 flex items-center gap-3">
                    <span className="text-2xl">
                      {
                        GAME_CATEGORIES.find((c) => c.key === libraryCategory)
                          ?.icon
                      }
                    </span>
                    <div>
          ...
          const getEloTitle = (elo: number) => {
            if (elo >= 2700)
              return {
                title: "SUPER GM",
                emoji: "👑",
                vibe: "Bro you play like Magnus??? No cap this is ELITE 🔥",
            ...
            if (elo >= 2500)
              return {
                title: "GRANDMASTER",
                emoji: "🏆",
                vibe: "Actual GM energy rn 😤 the opponents are NOT ready",
            ...
            if (elo >= 2300)
              return {
                title: "MASTER",
                emoji: "⚡",
                vibe: "Main character energy tbh 💅 lowkey cracked at chess",
            ...
            if (elo >= 2100)
              return {
                title: "EXPERT",
                emoji: "🎯",
                vibe: "This goes HARD 🔥 you're cooking and they can't handle it",
            ...
            if (elo >= 1900)
              return {
                title: "CLASS A",
                emoji: "💪",
                vibe: "Solid gameplay no cap 😤 you're on that sigma grindset",
            ...
            if (elo >= 1700)
              return {
                title: "CLASS B",
                emoji: "🧠",
                vibe: "POV: you chose violence on the board 😈 keep grinding bestie",
            ...
            if (elo >= 1500)
              return {
                title: "CLUB PLAYER",
                emoji: "♟️",
                vibe: "Not bad fr fr 💀 the vibes are immaculate, keep it up",
            ...
            if (elo >= 1300)
              return {
                title: "INTERMEDIATE",
                emoji: "📈",
                vibe: "The glow-up arc is starting 🌱 you're gonna be cracked soon",
            ...
            if (elo >= 1100)
              return {
                title: "IMPROVING",
                emoji: "🌟",
                vibe: "It's giving... potential bestie 💫 the grind doesn't stop",
            ...
            if (elo >= 900)
              return {
                title: "BEGINNER+",
                emoji: "🎮",
                vibe: "OK we all start somewhere 😭 but the hustle is real",
                color: "from-teal-400 to-emerald-500",
              };
            return {
              title: "NEWBIE",
              emoji: "🐣",
              vibe: "Fresh account energy 💀 but slay queen/king, you'll get there",
              ...
              >
                ...
                <div className="relative overflow-hidden bg-gradient-to-br from-purple-600/30 via-pink-600/20 to-blue-600/30 px-6 pt-8 pb-6">
                  ...
                  <div className="text-center">
                    <p className="text-xs font-bold uppercase tracking-widest text-purple-300/80">
                      ✨ Performance Report ✨
                    </p>
                ...
                <div className="space-y-3 p-6">
                  {/* White */}
                  <div
                    className={`rounded-2xl border border-white/[0.08] bg-gradient-to-r ${whiteInfo.color} bg-opacity-10 p-4`}
                    style={{
                      background: `linear-gradient(135deg, rgba(255,255,255,0.04), rgba(255,255,255,0.01))`,
                    }}
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-white/[0.1] text-2xl">
                        {whiteInfo.emoji}
                      </div>
                      <div className="flex-1">
                        ...
                        <div className="mt-0.5 flex items-baseline gap-2">
                          <span
                            className={`text-2xl font-black bg-gradient-to-r ${whiteInfo.color} bg-clip-text text-transparent`}
                          >
                            {whiteElo}
                          </span>
                          <span
                            className={`rounded-full bg-gradient-to-r ${whiteInfo.color} px-2 py-0.5 text-[10px] font-black text-white`}
                          >
                            {whiteInfo.title}
                          </span>
                        </div>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-white/[0.06] text-2xl">
                        {blackInfo.emoji}
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                        ...
                        <div className="mt-0.5 flex items-baseline gap-2">
                          <span
                            className={`text-2xl font-black bg-gradient-to-r ${blackInfo.color} bg-clip-text text-transparent`}
                          >
                            {blackElo}
                          </span>
                          <span
                            className={`rounded-full bg-gradient-to-r ${blackInfo.color} px-2 py-0.5 text-[10px] font-black text-white`}
                          >
                            {blackInfo.title}
                          </span>
                        </div>
                  ...
                  {gameHeaders.Result && (
                    <div className="text-center">
                      <span className="rounded-full border border-white/[0.08] bg-white/[0.04] px-4 py-1.5 text-sm font-black text-white">
                        {gameHeaders.Result === "1-0"
                          ? "⚪ White Wins"
                          : gameHeaders.Result === "0-1"
                            ? "⚫ Black Wins"
                            : "🤝 Draw"}
                      </span>
                    </div>
                  ...
                  <p className="text-center text-[10px] text-slate-600 pt-2">
                    🔥 firechess.com &middot; free game analysis powered by
                    Stockfish
                  </p>
                </div>
              </div>
