...
import { ChangelogSeenTracker } from "@/components/changelog-seen-tracker";
      ...
      <div className="mx-auto max-w-3xl px-5 py-16 md:px-10 md:py-24">
        ...
        <div className="mb-12 text-center">
          ...
          <p className="mt-3 text-sm text-slate-400">
            What&apos;s new in FireChess — {CHANGELOG.length} update
            {CHANGELOG.length !== 1 ? "s" : ""}, {totalChanges} changes
          </p>
          <Link
            href="/"
                ...
                <div className="space-y-2 px-5 pb-5 pt-4 md:px-6">
                  {entry.changes.map((change, ci) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-lg ${style.bg} text-sm`}
                        >
                          {icon}
                        </span>
                        <div className="min-w-0 flex-1">
                          <div className="mb-0.5 flex items-center gap-2">
                            <span
                              className={`rounded-md ${style.bg} px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider ${style.text}`}
                            >
                              {style.label}
                            </span>
                          </div>
