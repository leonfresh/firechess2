...
function InputPhase() {
  return (
    <div style={{ animation: "hpa-phase-enter 0.45s ease both" }}>
      ...
      <div className="mb-2.5 flex items-center overflow-hidden rounded-xl border border-orange-500/20 bg-[#ff5a1f]/[0.05]">
        ...
        {STEPS.map(({ icon, label }, idx) => {
          ...
          return (
            ...
            >
              ...
              <span
                className={`text-[10px] font-medium ${isDone ? "text-orange-400" : "text-[#565061]"}`}
              >
                {label}
              </span>
              {isDone && (
  ...
  return (
    <div style={{ animation: "hpa-phase-enter 0.45s ease both" }}>
      ...
      <div className="mb-3 flex gap-3">
        ...
        <div
          className="shrink-0 overflow-hidden rounded-lg border border-[#1e1a24] bg-[linear-gradient(180deg,rgba(7,11,28,0.96),rgba(30,43,90,0.88)_60%,rgba(236,72,153,0.26))] shadow-lg shadow-black/30"
          style={{
            width: 200,
            height: 200,
            animation: "hpa-fade-in 0.4s ease 0.3s both",
        ...
        <div className="flex flex-col justify-center gap-2.5">
          {[
            ...
            <div key={label} className="flex flex-col gap-0.5">
              <span className="flex items-center gap-1.5 text-[10px] text-[#f0edf2]">
                <span className={`h-2 w-2 rounded-full ${color}`} />
                {label}
              </span>
              <span className="ml-3.5 text-[8px] text-[#565061]">{sub}</span>
            </div>
          ))}
...
export function HeroProductAnimation({ paused }: { paused?: boolean }) {
  ...
  return (
    <div className="relative select-none">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-[#1e1a24] bg-[#0c0a09]/95 shadow-2xl shadow-black/40 backdrop-blur-sm">
        {/* Title bar */}
