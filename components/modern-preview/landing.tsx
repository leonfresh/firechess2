...
export function ModernLanding() {
  ...
  useEffect(() => {
    ...
    landingRef.current?.querySelectorAll("[data-reveal]").forEach(element => observer.observe(element));
    return () => observer.disconnect();
  }, []);
  const [showAllPlayers, setShowAllPlayers] = useState(false);
  const players = showAllPlayers ? [...featuredPlayers, ...remainingPlayers] : featuredPlayers;
  return <div ref={landingRef} className={`${s.root} ${s.landingRoot}`}>
    <PreviewHeader />
    <div className={s.landingContainer}>
      ...
      <section className={s.sampleSection} data-reveal aria-labelledby="sample-reports-heading">
      ...
      <LandingDetails /><section className={s.bottomCta} data-reveal><div><span className={s.eyebrow}>YOUR NEXT STEP</span><h2>Better chess starts with your chess.</h2><p>Start with a free scan. See what your games are telling you.</p></div><a href="#scan" className={s.primaryButton}>Find my patterns <ArrowDown size={17} /></a></section>
    </div>
  </div>;
}
