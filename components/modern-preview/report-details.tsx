...
import { ModernMentalGame } from "./mental-game";
...
export function ReportDetails({ scan, filter, hasProAccess }: { scan: PreviewScan; filter: string; hasProAccess: boolean }) {
  ...
  return <div className={s.detailStack}>
    ...
    {all && ai && <section className={s.insightSection}><header className={s.insightHeading}><h2>Coach’s assessment</h2><p>Strengths, weaknesses & next steps</p></header>{hasProAccess ? <div className={s.coachDetailGrid}>{[{ name: "Build on these", items: ai.strengths }, { name: "Focus your practice", items: ai.weaknesses }, { name: "Your next steps", items: ai.nextSteps }].map(group => <article key={group.name}><h3>{group.name}</h3><ul>{group.items.map(item => <li key={item}>{item}</li>)}</ul></article>)}</div> : <LockedDetails title="Your personalized improvement plan" description="Unlock the full strengths, weaknesses and next-step assessment." />}</section>}
    {(all || filter === "Openings") && <details className={s.detailDisclosure} key={`openings-${filter}`} open={filter === "Openings"}><summary>Opening repertoire <span>{r.leaks.length} recurring leaks · {r.oneOffMistakes.length} one-off mistakes · rankings & results</span></summary>{hasProAccess ? <div className={s.legacyDetails}>{r.openingSummaries?.length ? <OpeningRankings openingSummaries={r.openingSummaries} /> : <p>No opening rankings were stored with this scan.</p>}<PositionPerformance leaks={r.leaks} hasProAccess={hasProAccess} /></div> : <LockedDetails title="Your complete opening repertoire" description="Explore up to six opening findings below. Pro adds every finding, opening ranking and performance breakdown." />}</details>}
    {filter === "Tactics" && <article className={s.summaryCard}><span className={s.eyebrow}>{filter === "Tactics" ? "TACTICAL THEMES" : "POSITIONAL HABITS"}</span><h2>What keeps showing up?</h2><p>Tags can overlap: a single position may contain several themes. Search the findings below to explore a theme.</p><div className={s.tagCloud}>{[...tags].sort((a,b) => b[1]-a[1]).map(([tag,count]) => <span key={tag}>{tag}<b>{count}</b></span>)}</div></article>}
    {(all || filter === "Endgames") && endgame && <section className={s.insightSection}><header className={s.insightHeading}><h2>Endgame technique</h2><p>Conversion, defense & performance by piece type</p></header><div className={s.detailContent}><div className={s.summaryNumbers}><div><strong>{endgame.totalPositions}</strong><span>Positions analyzed</span></div><div><strong>{endgame.conversionRate == null ? "—" : `${endgame.conversionRate.toFixed(1)}%`}</strong><span>Winning positions converted</span></div><div><strong>{endgame.holdRate == null ? "—" : `${endgame.holdRate.toFixed(1)}%`}</strong><span>Worse positions held</span></div></div>{hasProAccess ? <><div className={s.tableScroll}><table className={s.dataTable}><caption>Endgame performance by type</caption><thead><tr><th>Endgame</th><th>Positions</th><th>Mistakes</th><th>Avg. evaluation loss</th></tr></thead><tbody>{endgame.byType.map(row => <tr key={row.type}><td>{row.type}{row.type === endgame.weakestType ? <small>Focus area</small> : null}</td><td>{row.count}</td><td>{row.mistakes}</td><td>{row.avgCpLoss >= 10000 ? "Mate swing" : (row.avgCpLoss/100).toFixed(2)}</td></tr>)}</tbody></table></div><p className={s.detailFootnote}>Loss is shown in pawn units. These stored averages include forced-mate evaluations, which can inflate the value.</p></> : <LockedDetails title="Endgame performance by piece type" description="Unlock the complete breakdown and every endgame position to review." />}</div></section>}
    {(all || filter === "Clock") && clock && <section className={s.insightSection}><header className={s.insightHeading}><h2>Time management</h2><p>{clock.gamesWithClockData} games with clock data · pacing & pressure</p></header><div className={s.detailContent}><div className={s.summaryNumbers}><div><strong>{clock.score}<small>/100</small></strong><span>Time management</span></div><div><strong>{clock.avgTimePerMove.toFixed(1)}<small>s</small></strong><span>Average per move</span></div><div><strong>{clock.timeScrambleCount}</strong><span>Games with time scrambles</span></div></div>{hasProAccess ? <div className={s.breakdownGrid}>{[["Justified thinks",clock.justifiedThinks],["Efficient moves",clock.efficientMoves],["Wasted thinks",clock.wastedThinks],["Rushed moves",clock.rushedMoves]].map(([label,value]) => <div key={label}><span>{label}</span><strong>{value}</strong></div>)}</div> : <LockedDetails title="Understand your pacing under pressure" description="Up to six clock moments are included below. Pro unlocks every moment and the full pacing breakdown." />}</div></section>}
    {(all || filter === "Positional") && r.structuralReport && <details className={s.detailDisclosure}><summary>Structures & winning positions <span>Pawn structures, king safety & castling choices</span></summary><div className={s.legacyDetails}><ScanStructuralStats report={r.structuralReport} /></div></details>}
    {all && r.mentalStats && <section className={s.insightSection}><header className={s.insightHeading}><h2>Mental game</h2><p>Streaks, resilience & results after a loss</p></header><div className={s.legacyDetails}><ModernMentalGame mentalStats={r.mentalStats} hasProAccess={hasProAccess} /></div></section>}
    <ReportExtras scan={scan} filter={filter} hasProAccess={hasProAccess} />
    {all && !hasProAccess && <LockedDetails title="Track your progress over time" description="Your Free plan includes this report. Pro history and comparisons help you see how your habits change across scans." />}
