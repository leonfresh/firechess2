...
function StudyPosition({ initialPractice = false, onFinish, pattern, onComplete, onNext, hasNext, onPrevious, positionIndex, positionCount, reportUrl }: { initialPractice?: boolean; onFinish?: () => void; onPrevious: () => void; positionIndex: number; positionCount: number; reportUrl: string; pattern: PreviewPattern; onComplete: (id: string) => void; onNext: () => void; hasNext: boolean }) {
  ...
  function tryMove(move: string | { from: string; to: string; promotion?: string }) {
    ...
    try {
      ...
      if (attempt.from === pattern.from && attempt.to === pattern.to && attempt.promotion === pattern.promotion) {
        setSolved(true); setMoveShown("best"); setFeedback(`That’s it — ${pattern.best}. ${themeHabit(theme)}`); setSelectedSquare(null); onComplete(pattern.id); return true;
      }
...
export function ModernReport({ scan }: { scan: PreviewScan }) {
  ...
  function startSession() {
    setTrainingSession(buildTrainingSession(patterns, completed)); setSessionIndex(0); setSessionSolved([]); setSessionFinished(false); setSessionRun(v => v + 1);
  }
  ...
  return <div className={s.root}><PreviewHeader report /><div className={s.reportLayout}>
    ...
    <div className={s.reportMain}><div className={s.reportBreadcrumb}><span>Analysis <ChevronRight size={13} />{sample ? "Sample report" : "Your report"}</span><button onClick={copyLink} className={s.quietButton}><Copy size={14} />Copy report link</button></div>{copyMessage && <p role="status" className={s.copyStatus}>{copyMessage}</p>}
      ...
      <section className={`${s.summaryCard} ${s.sessionCard}`} id="report-training">
        <span className={s.eyebrow}>YOUR ROAD TO 2000 · FREE TRAINING</span><h2>Turn this report into your next good habit.</h2><p>A focused session from your own positions: spot the threat, count attackers and defenders, then find the better move. Up to six positions, balanced across your themes.</p>
