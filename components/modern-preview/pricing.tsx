...
const COMPARISON = [
  ...
  ["Detailed repertoire & coaching","Preview","Full breakdown"], ["Report history & comparisons","Latest report","Full history"],
];
export function ModernPricing() {
  ...
  return <div className={s.root}><PreviewHeader /><div className={s.dashboardContainer}>
    ...
    <div className={s.accessNote}><p>This preview shows the proposed Free limits. Existing subscriptions and live entitlements have not changed.</p><Link href="/pricing">Current plan details →</Link></div>
    ...
    <section className={s.pricingComparison}><div className={s.sectionHeading}><div><span className={s.eyebrow}>THE DETAILS, SIDE BY SIDE</span><h2>What’s included?</h2></div><span className={s.muted}>Lifetime includes all Pro features.</span></div><div className={s.tableScroll}><table className={s.dataTable}><caption>Proposed limits for the redesigned experience</caption><thead><tr><th scope="col">Feature</th><th scope="col">Free</th><th scope="col">Pro & Lifetime</th></tr></thead><tbody>{COMPARISON.map(([feature,free,pro])=><tr key={feature}><th scope="row">{feature}</th><td>{free}</td><td>{pro}</td></tr>)}</tbody></table></div></section>
    <section className={s.faqSection}><div><span className={s.eyebrow}>BEFORE YOU CHOOSE</span><h2>Keep it simple.</h2><Link href="/account" className={s.textLink}>Manage an existing subscription →</Link></div><div className={s.faqList}>{[["Can I try the report first?","Yes. Explore a public sample report or start a free scan. The free examples include a board, move comparison and practice."],["What happens when I upgrade?","Your existing checkout securely handles payment through Stripe. Pro and Lifetime unlock the paid features supported by your account."],["Is Lifetime a subscription?","No. Lifetime is a one-time payment for Pro access. Monthly Pro is a recurring subscription."],["Will upgrading generate a new report?","Upgrading changes access; it does not automatically reanalyze your games. A fresh scan is needed for updated engine results or revised brilliant classifications."]].map(([question,answer])=><details key={question}><summary>{question}<span>+</span></summary><p>{answer}</p></details>)}</div></section>
  </div></div>;
}
