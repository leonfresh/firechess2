...
export function ChaosImpact({mate=false,pieces=[],kind}:{mate?:boolean;pieces?:string[];kind?:string}) {
  return <div className={styles.burst} data-mate={mate} data-kind={kind} aria-hidden="true">
    ...
    <span className={styles.label}>{mate?'CHECKMATE':kind==='nuclear'?'NUCLEAR!':kind==='promotion'?'PROMOTED!':kind==='power'?'POWER UP!':'BOOM!'}</span>
  </div>;
}
